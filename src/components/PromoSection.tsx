import Link from "next/link";
import RevealOnScroll from "./RevealOnScroll";
import ShopPromoCard from "./ShopPromoCard";
import type { Product } from "@/data/products";

type Promo = {
  id: string;
  imageUrl: string;
  mediaType: "IMAGE" | "VIDEO";
  title: string | null;
  subtitle: string | null;
  badgeText: string | null;
  sectionLabel: string | null;
  link: string | null;
  linkedProduct?: Product | null;
};

function PromoCard({ promo, accent }: { promo: Promo; accent: string }) {
  const card = (
    <div className="relative aspect-[4/3] rounded-3xl overflow-hidden bg-diose-black group">
      {promo.mediaType === "VIDEO" ? (
        <video
          src={promo.imageUrl}
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
          autoPlay
          muted
          loop
          playsInline
        />
      ) : (
        // eslint-disable-next-line @next/next/no-img-element
        <img
          src={promo.imageUrl}
          alt={promo.title ?? "Promoción"}
          className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
        />
      )}

      {promo.badgeText && (
        <div
          className="absolute top-3 right-3 text-white text-[11px] font-bold tracking-[0.02em] px-3 py-1.5 rounded-full rotate-3 shadow-strong"
          style={{ background: accent }}
        >
          {promo.badgeText}
        </div>
      )}

      {(promo.title || promo.subtitle) && (
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/85 to-transparent">
          {promo.title && <div className="text-white font-medium text-sm">{promo.title}</div>}
          {promo.subtitle && <div className="text-white/70 text-xs mt-0.5">{promo.subtitle}</div>}
        </div>
      )}
    </div>
  );
  return promo.link ? (
    <Link href={promo.link} className="block">
      {card}
    </Link>
  ) : (
    card
  );
}

const ACCENTS = [
  "var(--color-diose-red)",
  "var(--color-diose-blue)",
  "var(--color-diose-gold)",
  "var(--color-diose-green)",
  "var(--color-diose-purple)",
];

function SectionHeader({ label }: { label: string }) {
  return (
    <div className="rounded-2xl overflow-hidden mb-5">
      <div
        className="relative overflow-hidden py-2.5 px-5 flex items-center"
        style={{
          backgroundImage:
            "repeating-linear-gradient(135deg, var(--color-diose-black) 0 14px, var(--color-diose-blue) 14px 28px)",
        }}
      >
        <span className="bg-diose-black text-white text-[13px] md:text-[15px] font-bold tracking-[0.06em] uppercase px-4 py-1.5 rounded-full">
          {label}
        </span>
      </div>
    </div>
  );
}

export default function PromoSection({ promos }: { promos: Promo[] }) {
  if (promos.length === 0) return null;

  const grouped: { label: string | null; items: Promo[] }[] = [];
  for (const promo of promos) {
    const last = grouped[grouped.length - 1];
    if (last && last.label === promo.sectionLabel) {
      last.items.push(promo);
    } else {
      grouped.push({ label: promo.sectionLabel, items: [promo] });
    }
  }

  return (
    <section className="bg-white px-4 md:px-6 py-10 flex flex-col gap-12">
      <div className="max-w-7xl mx-auto w-full flex flex-col gap-12">
        {grouped.map((group, gi) => (
          <RevealOnScroll key={gi}>
            {group.label ? (
              <SectionHeader label={group.label} />
            ) : (
              <>
                <div className="text-[10px] font-semibold tracking-[0.16em] uppercase text-gray-400 mb-1.5">
                  Ofertas
                </div>
                <div className="font-heading text-2xl text-diose-black mb-5">Promociones</div>
              </>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {group.items.map((promo, i) =>
                promo.linkedProduct ? (
                  <ShopPromoCard
                    key={promo.id}
                    imageUrl={promo.imageUrl}
                    mediaType={promo.mediaType}
                    badgeText={promo.badgeText}
                    product={promo.linkedProduct}
                    accent={ACCENTS[i % ACCENTS.length]}
                  />
                ) : (
                  <PromoCard key={promo.id} promo={promo} accent={ACCENTS[i % ACCENTS.length]} />
                )
              )}
            </div>
          </RevealOnScroll>
        ))}
      </div>
    </section>
  );
}
